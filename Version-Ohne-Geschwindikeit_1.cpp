/*
Arduino Auto Tom & Willy - VEREINFACHTE VERSION
...
Informationen:
 ...
 T2 = A3 Links.
 regler = A0 Regler für die Geschwindigkeit.
*/

// Variablen
int geschwindigkeit;
int geschwindigkeit1;
int geschwindigkeit2;
...
void loop() {
  ...
  if (entfernung < 5 && entfernung > 0) {
    ...
    // Geschwindigkeitsregler - Potentiometer auslesen
    geschwindigkeit = analogRead(A0); // Geschwindigkeit am Potentiometer eingestellt
    geschwindigkeit1 = map(geschwindigkeit, 0, 1023, 0, 111);
    geschwindigkeit2 = map(geschwindigkeit, 0, 1023, 0, 127);
    ...
    if (links >= 550 && rechts >= 550) { // Logik Geradeaus Fahren
      ...
      analogWrite(3, geschwindigkeit1);
      analogWrite(5, geschwindigkeit2);
      Serial.println(geschwindigkeit1);
      Serial.println(geschwindigkeit2);
    }
