...
void loop() {
  ...
  if (entfernung <= 7){ // Rote LED als Warnung   
    digitalWrite(9, HIGH); // Rot
    digitalWrite(8, LOW);
    digitalWrite(10, LOW);
  }
  ...
  if (entfernung < 5 && entfernung > 0) {
    ...
    startgeschwindikeit1 = map(startgeschwindikeit, 0, 1023, 0, 120);
    ...
    if (links >= 550 && rechts >= 550) { // Logik Geradeaus Fahren
      ...
      analogWrite(3,angespasstegeschwindikeit1 ); // Max 225 geteilt durch 1.5
      kurve2 = angespasstegeschwindikeit2 / 2;
      analogWrite(5, kurve2);  // ehemalige Hälfte 127
      Serial.println("|||DAS AUTO FÄHRT RECHTS|||");
    }
    else if (rechts < 550) { // Logik Links Fahren (wenn T1 niedrig, T1 = Rechts)
      kurve1 = angespasstegeschwindikeit1 / 2;
      analogWrite(3, kurve1);  // 120
      analogWrite(5,angespasstegeschwindikeit2 ); // 255
